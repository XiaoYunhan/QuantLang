...
class QDplus:
    ...
    ref: https://github.com/antdvid/FastAmericanOptionPricing/blob/master/QDplusAmericanOptionSolver.py
    """
    ...
    def price(self, tau, S):
        ...
        if self.option_type == OptionType.Put:
            pS = EuropeanOption.european_put_value(tau, S, self.r, self.q, self.sigma, self.K)
            pSb = EuropeanOption.european_put_value(tau, Sb, self.r, self.q, self.sigma, self.K)
        else:
            pS = EuropeanOption.european_call_value(tau, S, self.r, self.q, self.sigma, self.K)
            pSb = EuropeanOption.european_call_value(tau, Sb, self.r, self.q, self.sigma, self.K)
    ...
    def compute_miscellaneous(self, tau, S):
        ...
        self.v_d1 = EuropeanOption.d1(tau, S, self.r, self.q, self.sigma, self.K)
        self.v_d2 = EuropeanOption.d2(tau, S, self.r, self.q, self.sigma, self.K)
        if self.option_type == OptionType.Put:
            self.v_p = EuropeanOption.european_put_value(tau, S, self.r, self.q, self.sigma, self.K)
        else:
            self.v_p = EuropeanOption.european_call_value(tau, S, self.r, self.q, self.sigma, self.K)
        self.v_theta = EuropeanOption.european_option_theta(tau, S, self.r, self.q, self.sigma, self.K)
